...
class DepartmentView(QWidget):
    ...
    def create_delete_screen(self):
        ...
        self.delete_table.horizontalHeader().setSectionResizeMode(
            1, QHeaderView.Stretch
        )
    ...
    def save_new_department(self):
        ...
        if not name:
            QMessageBox.warning(
                self, "Validation Error", "Department name is required!"
            )
            return
        ...
        if id:
            QMessageBox.information(
                self, "Success", f"Department '{name}' created successfully!"
            )
    ...
    def update_department(self):
        ...
        if not hasattr(self, "current_dept_id"):
            QMessageBox.warning(
                self, "No Selection", "Please select a department first!"
            )
            return
    ...
    def delete_department(self):
        ...
        if not selected:
            QMessageBox.warning(
                self, "No Selection", "Please select a department to delete!"
            )
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", "Department deleted successfully!"
                )
                self.stack.setCurrentIndex(0)
            else:
