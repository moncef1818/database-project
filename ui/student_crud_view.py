...
class StudentView(QWidget):
    ...
    def create_create_screen(self):
        ...
        self.create_address_input.setPlaceholderText(
            "Optional: e.g., 123 Rue de la Liberté"
        )
        ...
        self.create_email_input.setPlaceholderText(
            "Optional: e.g., ahmed.benali@example.com"
        )
    ...
    def save_new_student(self):
        ...
        if not last_name or not first_name:
            QMessageBox.warning(
                self, "Validation Error", "Last name and first name are required!"
            )
            return
        ...
        student_id = student_crud_queries.StudentCRUD.create_student(
            last_name, first_name, dob, address, city, zip_code, phone, fax, email
        )
    ...
    def perform_search(self):
        ...
        for row_idx, student in enumerate(results):
            ...
            self.read_table.setItem(
                row_idx, 1, QTableWidgetItem(student[1] or "")
            )  # Last name
            self.read_table.setItem(
                row_idx, 2, QTableWidgetItem(student[2] or "")
            )  # First name
            self.read_table.setItem(
                row_idx, 3, QTableWidgetItem(str(student[3]) if student[3] else "")
            )  # DOB
            self.read_table.setItem(
                row_idx, 4, QTableWidgetItem(student[4] or "")
            )  # Address
            self.read_table.setItem(
                row_idx, 5, QTableWidgetItem(student[5] or "")
            )  # City
            self.read_table.setItem(
                row_idx, 6, QTableWidgetItem(student[6] or "")
            )  # Zip code
            self.read_table.setItem(
                row_idx, 7, QTableWidgetItem(student[7] or "")
            )  # Phone
            self.read_table.setItem(
                row_idx, 8, QTableWidgetItem(student[8] or "")
            )  # Fax
            self.read_table.setItem(
                row_idx, 9, QTableWidgetItem(student[9] or "")
            )  # Email
    ...
    def delete_student(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", "Student deleted successfully!"
                )
                self.stack.setCurrentIndex(0)
            else:
    ...
    def show_update_screen(self):
        ...
        for row_idx, student in enumerate(results):
            ...
            self.update_table.setItem(
                row_idx, 3, QTableWidgetItem(str(student[3]) or "")
            )
